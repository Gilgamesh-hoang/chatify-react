...
const ChatUserMenu: React.FC<{
  ...
  const handleInfoClick = (evt: MouseEvent) => {
    onClose();
    onInfoClick();
  };

  const handleSendClick = (evt: MouseEvent) => {
    onClose();
    if (chatUserExist)
      navigate(`/0/${userName}`);
    else
  ...
  return (
    <div className="flex flex-col py-1">
      ...
      <button className="flex items-center mx-1 p-1 px-2 hover:bg-secondary" onClick={handleInfoClick}>
        <IoInformationCircle /><span className="pl-1">Check user infomation</span></button>

      <button className="flex items-center mx-1 p-1 px-2 hover:bg-secondary" onClick={handleSendClick}>
        {chatUserExist
          ? (<><IoChatbubbleEllipses /><span className="pl-1">Open chat messages</span></>)
          : (<><FaUserPlus /><span className="pl-1">Add users</span></>)}
